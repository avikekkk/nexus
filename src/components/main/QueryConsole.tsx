import { useCallback, useEffect, useMemo, useState } from "react"
import { useKeyboard } from "@opentui/react"
import { deleteWordBackward, getPrintableKey, isDeleteWordKey, isShiftEnterKey, isSubmitKey } from "../../utils/keyInput.ts"

interface QueryConsoleProps {
  focused: boolean
  query: string
  error?: string | null
  onChange: (query: string) => void
  onExecute: (query: string) => void
  onBlur: () => void
}

function clamp(value: number, min: number, max: number): number {
  return Math.max(min, Math.min(max, value))
}

export function QueryConsole({ focused, query, error, onChange, onExecute, onBlur }: QueryConsoleProps) {
  const [cursorPos, setCursorPos] = useState(query.length)

  useEffect(() => {
    setCursorPos((prev) => clamp(prev, 0, query.length))
  }, [query])

  const lines = useMemo(() => query.split("\n"), [query])
  const beforeCursor = query.slice(0, cursorPos)
  const cursorLine = beforeCursor.split("\n").length - 1
  const cursorColumn = beforeCursor.length - (beforeCursor.lastIndexOf("\n") + 1)

  const updateQuery = useCallback(
    (next: string, nextCursor: number) => {
      onChange(next)
      setCursorPos(clamp(nextCursor, 0, next.length))
    },
    [onChange]
  )

  useKeyboard((key) => {
    if (!focused) return

    if (key.name === "escape") {
      onBlur()
      return
    }

    if (isSubmitKey(key) && !isShiftEnterKey(key)) {
      onExecute(query)
      return
    }

    if (isShiftEnterKey(key)) {
      updateQuery(`${query.slice(0, cursorPos)}\n${query.slice(cursorPos)}`, cursorPos + 1)
      return
    }

    if (isDeleteWordKey(key)) {
      const result = deleteWordBackward(query, cursorPos)
      updateQuery(result.value, result.cursor)
      return
    }

    if (key.name === "left") {
      setCursorPos((prev) => Math.max(0, prev - 1))
      return
    }

    if (key.name === "right") {
      setCursorPos((prev) => Math.min(query.length, prev + 1))
      return
    }

    if (key.name === "home") {
      setCursorPos(0)
      return
    }

    if (key.name === "end") {
      setCursorPos(query.length)
      return
    }

    if (key.name === "backspace") {
      if (cursorPos <= 0) return
      updateQuery(`${query.slice(0, cursorPos - 1)}${query.slice(cursorPos)}`, cursorPos - 1)
      return
    }

    if (key.name === "delete") {
      if (cursorPos >= query.length) return
      updateQuery(`${query.slice(0, cursorPos)}${query.slice(cursorPos + 1)}`, cursorPos)
      return
    }

    const printable = getPrintableKey(key)
    if (printable) {
      updateQuery(`${query.slice(0, cursorPos)}${printable}${query.slice(cursorPos)}`, cursorPos + printable.length)
    }
  })

  return (
    <box flexGrow={1} flexDirection="column" padding={1}>
      <text fg="#565f89">Enter run • Shift+Enter newline • Esc exit input</text>
      <box height={1}>
        <text fg="#414868">{"─".repeat(200)}</text>
      </box>
      {error && (
        <text fg="#f7768e">{error}</text>
      )}
      <box flexGrow={1} flexDirection="column">
        {lines.length === 0 ? (
          <>
            <text fg="#565f89">Type query...</text>
            <text fg="#565f89">Mongo examples: db.users.find(&#123;&#125;)</text>
            <text fg="#565f89">db.users.find(&#123;"year": &#123;"$gte": 2000&#125;&#125;).sort(&#123;"year": -1&#125;).limit(20)</text>
          </>
        ) : (
          lines.map((line, index) => {
            if (!focused || index !== cursorLine) {
              return (
                <text key={index} fg="#a9b1d6">
                  {line || " "}
                </text>
              )
            }

            const safeColumn = clamp(cursorColumn, 0, line.length)
            const before = line.slice(0, safeColumn)
            const current = line[safeColumn]
            const after = line.slice(safeColumn + (current ? 1 : 0))

            return (
              <text key={index} fg="#c0caf5">
                {before}
                {current ? (
                  <span fg="#1a1b26" bg="#7aa2f7">
                    {current}
                  </span>
                ) : (
                  <span fg="#7aa2f7">█</span>
                )}
                {after}
              </text>
            )
          })
        )}
      </box>
    </box>
  )
}
